!  MODEL NAME : inj_IBG22rl             
!  MODEL DESCRIPTION FILE : IBG22rl.txt
!  Data :
!       prm(  1)=  Imax
!       prm(  2)=  IN
!       prm(  3)=  Iprate
...
!       prm( 35)=  pfup
!       prm( 36)=  pfdn
!       prm( 37)=  Trocof                          ! delay for ROCOF measurement
!       prm( 38)=  dfmax                           ! maximum permissable ROCOF
!       prm( 39)=  protection                      -1 to switch off, 1 to switch on
!       prm( 40)=  support
!  Parameters :
!       prm( 41)=  vxlv  
!       prm( 42)=  vylv  
...
subroutine inj_IBG22rl(nb,name,mode,nbxvar,nbzvar,nbdata,nbaddpar,prm,parname,nbobs, &
...
!vxlv = [vx] + {Re}*[ix] - {Xe}*[iy]
      prm( 41)= vx + prm( 18)*ix - prm( 19)*iy

!vylv = [vy] + {Re}*[iy] + {Xe}*[ix]
      prm( 42)= vy + prm( 18)*iy + prm( 19)*ix
...
!Pup = -{pfup}*{Pext}
      prm( 59)= -prm( 35)*prm( 44)

!Pdn = -{pfdn}*{Pext}
      prm( 60)= -prm( 36)*prm( 44)
...
!Iqmax =  dsqrt({Imax}**2 - {Ipref}**2 )
      x( 13)= dsqrt(prm(  1)**2 - prm( 47)**2 )

!Iqmin =  - dsqrt({Imax}**2 - {Ipref}**2 )
      x( 14)= - dsqrt(prm(  1)**2 - prm( 47)**2 )
...
!vq =  -{vxlv}*sin({theta_PLL}) + {vylv}*cos({theta_PLL})
      x( 20)= -prm( 41)*sin(prm( 49)) + prm( 42)*cos(prm( 49))

!vd =  {vxlv}*cos({theta_PLL}) + {vylv}*sin({theta_PLL})
      x( 21)= prm( 41)*cos(prm( 49)) + prm( 42)*sin(prm( 49))
...
!omegam =  -0.5*({vxlv}*sin({theta_PLL}) - {vylv}*cos({theta_PLL})) + 1
      x( 43)= -0.5*(prm( 41)*sin(prm( 49)) - prm( 42)*cos(prm( 49))) + 1

!fm =  50*(-0.5*({vxlv}*sin({theta_PLL}) - {vylv}*cos({theta_PLL})) + 1)
      x( 44)= 50*(-0.5*(prm( 41)*sin(prm( 49)) - prm( 42)*cos(prm( 49))) + 1)

!fmfilt =  50*(-0.5*({vxlv}*sin({theta_PLL}) - {vylv}*cos({theta_PLL})) + 1)
      x( 45)= 50*(-0.5*(prm( 41)*sin(prm( 49)) - prm( 42)*cos(prm( 49))) + 1)
...
!wpll =  -({vxlv}*sin({theta_PLL}) - {vylv}*cos({theta_PLL}))*{kpll} + 2*pi*50
      x( 55)= -(prm( 41)*sin(prm( 49)) - prm( 42)*cos(prm( 49)))*prm( 48) + 2*pi*50
...
!& tf1p2lim			! temporary current is
      ...
      if(1*x( 11)-x( 31)>prm( 55)*prm(  4))then
         z(  4)=1
      elseif(1*x( 11)-x( 31)<prm( 51)*prm(  4))then
         z(  4)=-1
      else
         z(  4)=0
      endif
...
!& tf1p2lim
      ...
      if(1*x( 59)-x( 58)>prm( 56)*prm( 58))then
         z( 12)=1
      elseif(1*x( 59)-x( 58)<prm( 51)*prm( 58))then
         z( 12)=-1
      else
         z( 12)=0
      endif
...
!& tf1p2lim			! temporary current is
      select case (z(  4))
         ...
         case(1)
            f( 14)=x( 84)-prm( 55)*prm(  4)
         case(-1)
            f( 14)=x( 84)-prm( 51)*prm(  4)
      end select
      select case (z(  5))
         case(0)
            f( 15)=x( 84)
...
!& tf1p2lim
      select case (z( 12))
         ...
         case(1)
            f( 35)=x( 86)-prm( 56)*prm( 58)
         case(-1)
            f( 35)=x( 86)-prm( 51)*prm( 58)
      end select
      select case (z( 13))
         case(0)
            f( 36)=x( 86)
...
!& algeq
      f( 68)=x( 65) - x( 64)*prm( 34)

!& algeq
      f( 69)=x( 66) - x( 64)*prm( 33)
...
!& algeq                ! frequency deviation in Hz
      f( 82)=-x( 71) + x( 45)-prm( 57)*50

!& tfder1p               ! Rocof measurement in Hz/s
      f( 83)=-x( 88)+x( 71)
      if (prm( 37)< 0.005)then
         f( 84)=1/prm( 37)*x( 71)-x( 72)
      else
         f( 84)=1/prm( 37)*(x( 71)-x( 88))-x( 72)
      endif
...
!& tf1p2lim			! temporary current is
      select case (z(  4))
         case(0)
            if(x( 84)>prm( 55)*prm(  4))then
               z(  4)=1
            elseif(x( 84)<prm( 51)*prm(  4))then
               z(  4)=-1
            endif
         case(1)
            if(1*x( 11)-x( 31)<prm( 55)*prm(  4))then
         ...
         case(-1)
            if(1*x( 11)-x( 31)>prm( 51)*prm(  4))then
               z(  4)= 0
            endif
      end select
      select case (z(  5))
...
!& tf1p2lim
      select case (z( 12))
         case(0)
            if(x( 86)>prm( 56)*prm( 58))then
               z( 12)=1
            elseif(x( 86)<prm( 51)*prm( 58))then
               z( 12)=-1
            endif
         case(1)
            if(1*x( 59)-x( 58)<prm( 56)*prm( 58))then
         ...
         case(-1)
            if(1*x( 59)-x( 58)>prm( 51)*prm( 58))then
               z( 12)= 0
            endif
      end select
      select case (z( 13))
